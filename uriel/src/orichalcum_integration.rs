...
impl AsyncNodeLogic for GeminiIngestionNode {
    ...
    async fn exec(&self, input_val: JsonValue) -> JsonValue {
        ...
        let entities: Vec<String> = input_val.get("entities")
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
            .unwrap_or_default();

        let prompt_text = format!("Classify the following text into one of these intents: Note, Draft, Event, Disambiguate. \
        Available entities: {:?}. If user mentions an entity, match exactly. If ambiguous, set intent to Disambiguate. \
        Return JSON strictly matching this schema: {{ 'intent': '...', 'target_folder': '...', 'entities_found': [...], 'formatted_content': '...' }}. \
        Text: {}", entities, input_text);
